...
<body class="bg-white text-[#0f1419] min-h-screen">
    ...
    <header class="sticky top-0 z-50 bg-white border-b border-[#cfd9de] px-4 lg:px-8 py-3">
        <div class="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
            <div class="flex items-center space-x-3">
                <div class="w-9 h-9 rounded-lg bg-[#0f1419] text-white flex items-center justify-center font-bold text-base">
                    H
                </div>
                <div>
                    <div class="flex items-center space-x-2">
                        <h1 class="text-base font-bold tracking-tight text-[#0f1419]">
                            VEXUM CFO Intelligence Portal
                        </h1>
                        <span id="header-badge" class="px-2 py-0.5 text-xs font-semibold rounded bg-[#f7f9f9] text-[#0f1419] border border-[#cfd9de]">
                            同期中
                        </span>
                    </div>
                    <p class="text-xs text-[#536471]">
            ...
            <div class="flex items-center space-x-2">
                <button onclick="exportDataJSON()" class="px-3 py-1.5 rounded-lg bg-white border border-[#cfd9de] hover:bg-[#f7f9f9] text-[#0f1419] transition text-xs font-semibold flex items-center space-x-1.5" title="データエクスポート">
                    <i data-lucide="download" class="w-4 h-4 text-[#0f1419]"></i>
                    <span>JSON保存</span>
                </button>
                <button onclick="switchTab('tab-manage')" class="px-3.5 py-1.5 rounded-lg bg-[#0f1419] hover:bg-[#272c30] text-white transition text-xs font-semibold flex items-center space-x-1.5 shadow-sm">
        ...
        <!-- Navigation Tabs -->
        <div class="max-w-7xl mx-auto mt-3 overflow-x-auto flex space-x-1 border-t border-[#cfd9de] pt-2 text-xs font-bold">
            <button onclick="switchTab('tab-profile')" id="btn-tab-profile" class="tab-btn active px-4 py-2 rounded-lg transition flex items-center space-x-1.5 whitespace-nowrap text-[#536471] hover:text-[#0f1419] hover:bg-[#f7f9f9]">
                <i data-lucide="user-check" class="w-3.5 h-3.5"></i>
                <span>1. 人物像 & CFO機能の全貌</span>
            </button>
            <button onclick="switchTab('tab-kpi')" id="btn-tab-kpi" class="tab-btn px-4 py-2 rounded-lg transition flex items-center space-x-1.5 whitespace-nowrap text-[#536471] hover:text-[#0f1419] hover:bg-[#f7f9f9]">
                <i data-lucide="activity" class="w-3.5 h-3.5"></i>
                <span>2. 必見KPI & 逆算シミュレーター</span>
            </button>
            <button onclick="switchTab('tab-chronicle')" id="btn-tab-chronicle" class="tab-btn px-4 py-2 rounded-lg transition flex items-center space-x-1.5 whitespace-nowrap text-[#536471] hover:text-[#0f1419] hover:bg-[#f7f9f9]">
                <i data-lucide="calendar-days" class="w-3.5 h-3.5"></i>
                <span>3. 経営会議 完全クロニクル</span>
            </button>
            <button onclick="switchTab('tab-roadmap')" id="btn-tab-roadmap" class="tab-btn px-4 py-2 rounded-lg transition flex items-center space-x-1.5 whitespace-nowrap text-[#536471] hover:text-[#0f1419] hover:bg-[#f7f9f9]">
                <i data-lucide="graduation-cap" class="w-3.5 h-3.5"></i>
                <span>4. 「第二の日置さん」育成ロードマップ</span>
            </button>
            <button onclick="switchTab('tab-manage')" id="btn-tab-manage" class="tab-btn px-4 py-2 rounded-lg transition flex items-center space-x-1.5 whitespace-nowrap text-[#536471] hover:text-[#0f1419] hover:bg-[#f7f9f9]">
                <i data-lucide="upload-cloud" class="w-3.5 h-3.5"></i>
    ...
    <!-- Main Content Area -->
    <main class="max-w-7xl mx-auto px-4 lg:px-8 py-8 space-y-8 bg-white">

        <!-- TAB 1: Profile -->
        <section id="tab-profile" class="tab-content block space-y-8">
            <div class="bg-white border border-[#cfd9de] rounded-2xl p-6 lg:p-8 space-y-6">
                <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                    <div class="space-y-3 max-w-3xl">
                        <div class="inline-flex items-center space-x-2 px-2.5 py-1 rounded bg-[#f7f9f9] text-[#0f1419] text-xs font-bold border border-[#cfd9de]">
                            <span>株式会社VEXUM 代表取締役 兼 経営統括・CFO</span>
                        </div>
                        <h2 class="text-3xl font-extrabold text-[#0f1419] tracking-tight">
                            日置 佑輔 <span class="text-lg font-normal text-[#536471]">（Yusuke）</span>
                        </h2>
                        <p class="text-base text-[#0f1419] font-bold leading-relaxed">
                            「数字を共通言語にして、逆算とスピードで“成長の再現性”を証明し続ける」
                        </p>
                        <p class="text-xs text-[#0f1419] leading-relaxed font-normal">
                    ...
                    <div class="grid grid-cols-2 gap-3 w-full md:w-auto shrink-0 font-mono text-center">
                        <div class="bg-[#f7f9f9] border border-[#cfd9de] p-3.5 rounded-xl">
                            <div class="text-[11px] text-[#536471] font-sans font-bold">全社目標 (1年後)</div>
                            <div class="text-lg font-extrabold text-[#0f1419]">有料 3,000社</div>
                            <div class="text-[10px] text-[#536471] font-sans font-medium">1億円ボーナス</div>
                        </div>
                        <div class="bg-[#f7f9f9] border border-[#cfd9de] p-3.5 rounded-xl">
                            <div class="text-[11px] text-[#536471] font-sans font-bold">資金調達目標</div>
                            <div class="text-lg font-extrabold text-[#0f1419]">5億〜10億円</div>
                            <div class="text-[10px] text-[#536471] font-sans font-medium">シリーズA</div>
                        </div>
                        <div class="bg-[#f7f9f9] border border-[#cfd9de] p-3.5 rounded-xl">
                            <div class="text-[11px] text-[#536471] font-sans font-bold">蓄積会議数</div>
                            <div id="summary-meeting-count" class="text-lg font-extrabold text-[#0f1419]">-</div>
                            <div class="text-[10px] text-[#536471] font-sans font-medium">全件解析済み</div>
                        </div>
                        <div class="bg-[#f7f9f9] border border-[#cfd9de] p-3.5 rounded-xl">
                            <div class="text-[11px] text-[#536471] font-sans font-bold">意思決定ルール</div>
                            <div class="text-lg font-extrabold text-[#0f1419]">10分会議</div>
                            <div class="text-[10px] text-[#536471] font-sans font-medium">LINE即時承認</div>
                        </div>
                    </div>
                </div>

                <div class="bg-[#f7f9f9] border-l-4 border-[#0f1419] p-3.5 rounded-r-lg flex items-start space-x-3 border border-[#cfd9de]">
                    <i data-lucide="sparkles" class="w-4 h-4 text-[#0f1419] shrink-0 mt-0.5"></i>
                    <div class="text-xs space-y-1">
                        <div class="font-bold text-[#0f1419]">最新の経営進捗・日置さんの重要方針（<span id="latest-meeting-date">-</span>）：</div>
                        <div id="latest-decision-text" class="text-[#0f1419] leading-relaxed font-normal">-</div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-4">
                <div class="flex items-center space-x-2">
                    <i data-lucide="cpu" class="w-5 h-5 text-[#0f1419]"></i>
                    <h3 class="text-xl font-bold text-[#0f1419]">日置流『思考OS（Thinking OS）』7大原則</h3>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div class="bg-white border border-[#cfd9de] p-5 rounded-xl flex flex-col justify-between space-y-3">
                        <div>
                            <div class="text-xs font-bold text-[#536471] font-mono mb-1">原則 01</div>
                            <h4 class="text-base font-bold text-[#0f1419] mb-2">すべては「逆算」で組む</h4>
                            <p class="text-xs text-[#0f1419] leading-relaxed font-normal">
                                ゴール（例：7月上駐100件）からリードタイム・各ファネル転換率・必要行動量へ分解。「上駐100件 ÷ 6掛け ＝ アポ160件 ＝ 初回接触200件」を即座に組み立てる。
                            </p>
                        </div>
                        <div class="pt-2 border-t border-[#e1e8ed] text-[11px] text-[#536471] font-medium">
                    ...
                    <div class="bg-white border border-[#cfd9de] p-5 rounded-xl flex flex-col justify-between space-y-3">
                        <div>
                            <div class="text-xs font-bold text-[#536471] font-mono mb-1">原則 02</div>
                            <h4 class="text-base font-bold text-[#0f1419] mb-2">定性論を数字に翻訳させる</h4>
                            <p class="text-xs text-[#0f1419] leading-relaxed font-normal">
                    ...
                    <div class="bg-white border border-[#cfd9de] p-5 rounded-xl flex flex-col justify-between space-y-3">
                        <div>
                            <div class="text-xs font-bold text-[#536471] font-mono mb-1">原則 03</div>
                            <h4 class="text-base font-bold text-[#0f1419] mb-2">成長の再現性・スケールの証明</h4>
                            <p class="text-xs text-[#0f1419] leading-relaxed font-normal">
                    ...
                    <div class="bg-white border border-[#cfd9de] p-5 rounded-xl flex flex-col justify-between space-y-3">
                        <div>
                            <div class="text-xs font-bold text-[#536471] font-mono mb-1">原則 04</div>
                            <h4 class="text-base font-bold text-[#0f1419] mb-2">スピード＝唯一の競争優位</h4>
                            <p class="text-xs text-[#0f1419] leading-relaxed font-normal">
                    ...
                    <div class="bg-white border border-[#cfd9de] p-5 rounded-xl flex flex-col justify-between space-y-3">
                        <div>
                            <div class="text-xs font-bold text-[#536471] font-mono mb-1">原則 05</div>
                            <h4 class="text-base font-bold text-[#0f1419] mb-2">予実管理と「見込み3倍」ルール</h4>
                            <p class="text-xs text-[#0f1419] leading-relaxed font-normal">
                    ...
                    <div class="bg-white border border-[#cfd9de] p-5 rounded-xl flex flex-col justify-between space-y-3">
                        <div>
                            <div class="text-xs font-bold text-[#536471] font-mono mb-1">原則 06</div>
                            <h4 class="text-base font-bold text-[#0f1419] mb-2">利益構造・ユニットエコノミクス</h4>
                            <p class="text-xs text-[#0f1419] leading-relaxed font-normal">
        ...
        <section id="tab-kpi" class="tab-content hidden space-y-8">
            <div class="bg-white border border-[#cfd9de] rounded-2xl p-6 lg:p-8 space-y-6">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-2 border-b border-[#e1e8ed] pb-4">
                    <div>
                        <div class="text-xs font-bold uppercase tracking-wider text-[#536471]">
                            Interactive Reverse Engineering Tool
                        </div>
                        <h3 class="text-xl font-bold text-[#0f1419]">
                            日置流『営業・供給 逆算シミュレーター』
                        </h3>
                    </div>
                    <p class="text-xs text-[#536471] max-w-md">
                        目標とする「上駐開始件数（成約）」を入力すると、日置さんのロジック（転換率・リードタイム・リーダーキャパ）に基づいて必要なアクション量を即座に逆算します。
                    </p>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="space-y-4 bg-[#f7f9f9] p-4 rounded-xl border border-[#cfd9de]">
                        <div>
                            <label class="block text-xs font-bold text-[#0f1419] mb-1">
                                目標 上駐開始数 (成約件数 / 月)
                            </label>
                            <div class="relative">
                                <input type="number" id="sim-target-deals" value="100" class="w-full bg-white border border-[#cfd9de] rounded-lg px-3 py-2 text-[#0f1419] font-mono font-bold text-lg focus:border-[#0f1419] focus:outline-none" oninput="calculateReverseModel()">
                                <span class="absolute right-3 top-2.5 text-xs text-[#536471] font-bold">社</span>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="range" id="sim-rate-deal" min="30" max="90" value="63" class="w-full accent-[#0f1419]" oninput="calculateReverseModel()">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="range" id="sim-rate-design" min="40" max="95" value="80" class="w-full accent-[#0f1419]" oninput="calculateReverseModel()">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" id="sim-leader-cap" value="8" class="w-full bg-white border border-[#cfd9de] rounded-lg px-3 py-1.5 text-[#0f1419] font-mono text-sm focus:outline-none" oninput="calculateReverseModel()">
                        </div>
                    </div>

                    <div class="md:col-span-2 grid grid-cols-2 sm:grid-cols-3 gap-3">
                        <div class="bg-[#f7f9f9] border border-[#cfd9de] p-4 rounded-xl flex flex-col justify-between">
                            <div class="text-xs font-bold text-[#0f1419]">① 必要初期設計数 (先行指標)</div>
                            <div id="res-design-count" class="text-3xl font-extrabold text-[#0f1419] font-mono my-2">160</div>
                            <div class="text-[10px] text-[#536471] font-medium">リードタイム: 約22日前</div>
                        </div>

                        <div class="bg-[#f7f9f9] border border-[#cfd9de] p-4 rounded-xl flex flex-col justify-between">
                            <div class="text-xs font-bold text-[#0f1419]">② 必要アポイント数</div>
                            <div id="res-appo-count" class="text-3xl font-extrabold text-[#0f1419] font-mono my-2">200</div>
                            <div id="res-appo-daily" class="text-[10px] text-[#536471] font-mono font-medium">日次: 約8.0件</div>
                        </div>

                        <div class="bg-[#f7f9f9] border border-[#cfd9de] p-4 rounded-xl flex flex-col justify-between">
                            <div class="text-xs font-bold text-[#0f1419]">③ 必要見込み (3倍ルール)</div>
                            <div id="res-pipeline-3x" class="text-3xl font-extrabold text-[#0f1419] font-mono my-2">300</div>
                            <div class="text-[10px] text-[#536471] font-bold">下振れ防止バッファ</div>
                        </div>

                        <div class="bg-[#f7f9f9] border border-[#cfd9de] p-4 rounded-xl flex flex-col justify-between">
                            <div class="text-xs font-bold text-[#0f1419]">④ 必要リーダー数</div>
                            <div id="res-leader-count" class="text-3xl font-extrabold text-[#0f1419] font-mono my-2">13</div>
                            <div class="text-[10px] text-[#536471] font-medium">名 (案件管理体制)</div>
                        </div>

                        <div class="bg-[#f7f9f9] border border-[#cfd9de] p-4 rounded-xl flex flex-col justify-between">
                            <div class="text-xs font-bold text-[#0f1419]">⑤ 想定月次売上 (単価25万)</div>
                            <div id="res-monthly-sales" class="text-3xl font-extrabold text-[#0f1419] font-mono my-2">2,500万</div>
                            <div class="text-[10px] text-[#536471] font-medium">円 / 月</div>
                        </div>

                        <div class="bg-[#f7f9f9] border border-[#cfd9de] p-4 rounded-xl flex flex-col justify-between">
                            <div class="text-xs font-bold text-[#0f1419]">⑥ インターン給与上限 (50%)</div>
                            <div id="res-intern-budget" class="text-3xl font-extrabold text-[#0f1419] font-mono my-2">1,250万</div>
                            <div class="text-[10px] text-[#536471] font-medium">円 (財務規律上限)</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <section id="tab-chronicle" class="tab-content hidden space-y-6">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h3 class="text-xl font-bold text-[#0f1419] tracking-tight">
                        VEXUM経営会議 完全クロニクル
                    </h3>
                    <p class="text-xs text-[#536471]">
                        蓄積されたすべての会議議事録・意思決定ログ（全 <span id="chronicle-total-count" class="font-bold text-[#0f1419]">0</span> 件）
                    </p>
                </div>
                <div class="flex items-center gap-2">
                    <input type="text" id="chronicle-search" placeholder="キーワード・発言検索..." class="bg-white border border-[#cfd9de] rounded-lg px-3 py-1.5 text-xs text-[#0f1419] placeholder-[#536471] focus:border-[#0f1419] focus:outline-none" oninput="filterChronicle()">
                    <select id="chronicle-tag-filter" class="bg-white border border-[#cfd9de] rounded-lg px-2.5 py-1.5 text-xs text-[#0f1419] focus:outline-none" onchange="filterChronicle()">
        ...
        <section id="tab-roadmap" class="tab-content hidden space-y-8">
            ...
            <div class="bg-white border border-[#cfd9de] rounded-xl p-5 space-y-4">
                <h4 class="text-sm font-bold text-[#0f1419] flex items-center space-x-2">
            ...
            <div class="bg-white border border-[#cfd9de] rounded-xl p-6 space-y-6">
                <div class="flex items-center justify-between border-b border-[#e1e8ed] pb-3">
                    <h4 class="text-base font-bold text-[#0f1419]">
                        日置式 実践思考ドリル (<span id="drill-score" class="font-bold text-[#0f1419]">0</span> / 5 正解)
                    </h4>
                </div>
        ...
        <section id="tab-manage" class="tab-content hidden space-y-8">
            ...
            <div id="drop-zone" class="bg-white border-2 border-dashed border-[#cfd9de] hover:border-[#0f1419] p-8 rounded-2xl text-center space-y-4 transition-all cursor-pointer" onclick="document.getElementById('file-drop-input').click()">
                <input type="file" id="file-drop-input" accept=".pdf,.txt" multiple class="hidden" onchange="handleFileDropSelect(event)">
                <div class="w-14 h-14 rounded-xl bg-[#f7f9f9] text-[#0f1419] flex items-center justify-center mx-auto border border-[#cfd9de]">
                ...
                <div id="drop-loading-status" class="hidden text-xs font-bold text-[#0f1419] animate-pulse">
    ...
    <footer class="border-t border-[#cfd9de] mt-16 py-8 px-4 text-center text-xs text-[#536471] bg-white">
    ...
    <script>
                ...
                chronicleData.forEach(m => {{
                    (m.yusuke_decisions || []).forEach(d => {{
                        if (count < 10) {{
                            ...
                            item.className = 'bg-[#f7f9f9] p-2.5 rounded border border-[#cfd9de] space-y-1';
                            item.innerHTML = `<div class="text-[10px] font-bold text-[#536471]">${{m.display_date || m.date}}</div><div class="text-[#0f1419]">${{d}}</div>`;
        ...
        function renderChronicleCards(dataToRender) {{
            ...
            dataToRender.forEach((m, idx) => {{
                ...
                card.className = "bg-white border border-[#cfd9de] rounded-xl p-5 space-y-3";
                
                const tagsHtml = (m.tags || []).map(t => `<span class="px-2 py-0.5 rounded text-[10px] bg-[#f7f9f9] border border-[#cfd9de] text-[#0f1419] font-medium">${{t}}</span>`).join(' ');
                const decisionsHtml = (m.yusuke_decisions || []).map(d => `<li>${{d}}</li>`).join('');
                const quotesHtml = (m.quotes || []).map(q => `
                    <div class="bg-[#f7f9f9] border-l-2 border-[#0f1419] p-2.5 rounded text-xs text-[#0f1419] italic font-medium">
                        💬 "${{q}}"
                    </div>
                `).join('');

                card.innerHTML = `
                    <div class="flex items-center justify-between border-b border-[#e1e8ed] pb-2">
                        <div class="flex items-center space-x-2">
                            <span class="px-2 py-0.5 rounded bg-[#f7f9f9] text-[#0f1419] text-xs font-bold font-mono border border-[#cfd9de]">
                                第${{idx + 1}}回
                            </span>
                            <span class="text-xs font-bold text-[#0f1419]">
                    ...
                    <h4 class="text-sm font-bold text-[#0f1419]">${{m.theme}}</h4>
                    <p class="text-xs text-[#0f1419] leading-relaxed font-normal">${{m.summary}}</p>
                    ${{decisionsHtml ? `
                    <div class="space-y-1 pt-2 border-t border-[#e1e8ed]">
                        <div class="text-[11px] font-bold text-[#0f1419]">日置さんの重要意思決定・指摘：</div>
                        <ul class="text-xs text-[#0f1419] space-y-1 list-disc list-inside font-normal">
                            ${{decisionsHtml}}
                        </ul>
                    </div>` : ''}}
        ...
        function renderDrills() {{
            ...
            drillsData.forEach(d => {{
                const card = document.createElement('div');
                card.className = "bg-[#f7f9f9] p-5 rounded-xl border border-[#cfd9de] space-y-3";
                card.id = `drill-${{d.id}}`;

                const optionsHtml = d.options.map(opt => `
                    <button onclick="handleAnswerDrill(${{d.id}}, '${{opt.key}}')" class="w-full text-left p-3 rounded-lg bg-white border border-[#cfd9de] hover:bg-[#f7f9f9] text-[#0f1419] text-xs font-medium transition">
                ...
                card.innerHTML = `
                    <div class="flex items-center justify-between">
                        <span class="text-xs font-bold px-2 py-0.5 rounded bg-white text-[#0f1419] border border-[#cfd9de] font-mono">
                            Q${{d.id}}. ${{d.category}}
                        </span>
                        <span class="text-xs font-bold text-[#536471]" id="drill-${{d.id}}-status">未回答</span>
                    </div>
                    <p class="text-sm font-bold text-[#0f1419]">${{d.question}}</p>
        ...
        function handleAnswerDrill(qId, choice) {{
            ...
            if (choice === drill.correct) {{
                resultEl.className = 'p-3 rounded-lg text-xs leading-relaxed bg-white text-[#0f1419] border border-[#cfd9de] font-medium';
                resultEl.innerHTML = `<div class="font-bold text-[#0f1419] mb-1">正解！ 正しい日置さんの思考です。</div>${{drill.explanation}}`;
                statusEl.textContent = '正解 ✅';
                drillResults[qId] = true;
            }} else {{
                resultEl.className = 'p-3 rounded-lg text-xs leading-relaxed bg-rose-50 text-rose-900 border border-rose-200 font-medium';
                resultEl.innerHTML = `<div class="font-bold text-rose-900 mb-1">不正解（正解: ${{drill.correct}}）</div>${{drill.explanation}}`;
