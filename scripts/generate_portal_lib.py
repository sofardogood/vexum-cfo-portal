...
def get_header_html(active_index, total_count):
    navs = [
        ("index.html", "1. 人物像 & CFO思考OS"),
        ("2_必見KPI_逆算シミュレーター.html", "2. 必見KPI & 逆算シミュレーター"),
        ("3_全会議クロニクル.html", f"3. 全会議クロニクル ({total_count})"),
        ("4_第二の日置さん_思考ドリル.html", "4. 思考ドリル"),
        ("5_ドキュメント投入_同期.html", "5. ドキュメント投入 / 同期")
    ]

    nav_links = ""
    for idx, (href, title) in enumerate(navs):
        active_class = "bg-slate-900 text-white font-semibold" if idx == active_index else "text-slate-600 hover:text-slate-900 hover:bg-slate-50"
        nav_links += f'<a href="{href}" class="px-3.5 py-1.5 rounded-md transition whitespace-nowrap {active_class}">{title}</a>\n'

    return f'''
    <header class="sticky top-0 z-50 bg-white border-b border-slate-200 px-4 lg:px-8 py-3.5">
        <div class="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
            <div class="flex items-center space-x-3">
                <a href="index.html" class="w-9 h-9 rounded-lg bg-slate-900 text-white flex items-center justify-center font-bold text-base hover:bg-slate-800 transition">
                    H
                </a>
                <div>
                    <div class="flex items-center space-x-2">
                        <a href="index.html" class="text-base font-bold tracking-tight text-slate-900">
            ...
            <div class="flex items-center space-x-2">
                <a href="5_ドキュメント投入_同期.html" class="px-3.5 py-1.5 rounded-lg bg-slate-900 hover:bg-slate-800 text-white transition text-xs font-semibold flex items-center space-x-1.5">
...
def get_footer_html():
    ...
    <footer class="border-t border-slate-200 mt-16 py-6 px-4 text-center text-xs text-slate-400 bg-white">
        <p>株式会社VEXUM CFO Intelligence System | Multi-page Portal Structure</p>
    </footer>
    '''

def generate_html(chronicle_data, main_output_path):
    project_root = os.path.dirname(main_output_path)
    total_count = len(chronicle_data)
    chronicle_json_str = json.dumps(chronicle_data, ensure_ascii=False)
    drills_data = get_drills_data()
    drills_json_str = json.dumps(drills_data, ensure_ascii=False)

    latest = chronicle_data[0] if chronicle_data else {}
    latest_date = latest.get('display_date', latest.get('date', '2026年8月最新'))
    latest_dec = (latest.get('yusuke_decisions') and latest.get('yusuke_decisions')[0]) or latest.get('summary', '経営数値の進捗確認と重要意思決定を推進中。')
...
<head>
    ...
    <title>{title} | VEXUM CFO Intelligence Portal</title>
...
'''
    ...
    # ==========================================
    # PAGE 1: index.html & 日置さん_CFOインテリジェンス_ポータル.html
    # ==========================================
    p1_content = f'''{get_head("1. 人物像 & CFO思考OS")}
    {get_header_html(0, total_count)}
    <main class="max-w-6xl mx-auto px-4 lg:px-8 py-8 space-y-8 bg-white">
        <div class="border border-slate-200 rounded-xl p-6 lg:p-8 space-y-6">
            <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
                ...
                <div class="grid grid-cols-2 gap-3 w-full lg:w-auto shrink-0 text-center font-mono">
                    ...
                    <div class="border border-slate-200 p-3.5 rounded-lg bg-white">
                        ...
                        <div class="text-xl font-bold text-slate-900 mt-0.5">{total_count} 回分</div>
            ...
            <div class="border-t border-slate-100 pt-4 flex items-start space-x-2 text-xs">
                <span class="font-bold text-slate-900 shrink-0">最新の重要方針（{latest_date}）：</span>
                <span class="text-slate-700 leading-relaxed">{latest_dec}</span>
            </div>
        </div>
        ...
        <div class="space-y-4">
            <div class="flex items-center justify-between border-b border-slate-100 pb-2">
                ...
                <a href="2_必見KPI_逆算シミュレーター.html" class="text-xs font-semibold text-slate-900 hover:text-blue-600 flex items-center space-x-1">
        ...
        <div class="flex justify-between items-center pt-6 border-t border-slate-100">
            <div class="text-xs text-slate-500">ページ 1 / 5</div>
            <a href="2_必見KPI_逆算シミュレーター.html" class="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-xs font-semibold flex items-center space-x-1.5">
...
'''
    ...
    # ==========================================
    # PAGE 2: 2_必見KPI_逆算シミュレーター.html
    # ==========================================
    p2_content = f'''{get_head("2. 必見KPI & 逆算シミュレーター")}
    {get_header_html(1, total_count)}
    <main class="max-w-6xl mx-auto px-4 lg:px-8 py-8 space-y-8 bg-white">
        <div class="space-y-1">
        ...
        <div class="flex justify-between items-center pt-6 border-t border-slate-100">
            <a href="index.html" class="px-4 py-2 border border-slate-200 hover:bg-slate-50 text-slate-700 rounded-lg text-xs font-semibold flex items-center space-x-1.5">
                <i data-lucide="arrow-left" class="w-3.5 h-3.5"></i>
                <span>前へ: 1. 人物像 & CFO思考OS</span>
            </a>
            <a href="3_全会議クロニクル.html" class="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-xs font-semibold flex items-center space-x-1.5">
...
'''
    ...
    # ==========================================
    # PAGE 3: 3_全会議クロニクル.html
    # ==========================================
    p3_content = f'''{get_head("3. 全会議クロニクル")}
    {get_header_html(2, total_count)}
    <main class="max-w-6xl mx-auto px-4 lg:px-8 py-8 space-y-8 bg-white">
        ...
        <div class="flex justify-between items-center pt-6 border-t border-slate-100">
            <a href="2_必見KPI_逆算シミュレーター.html" class="px-4 py-2 border border-slate-200 hover:bg-slate-50 text-slate-700 rounded-lg text-xs font-semibold flex items-center space-x-1.5">
                <i data-lucide="arrow-left" class="w-3.5 h-3.5"></i>
                <span>前へ: 2. 必見KPI & 逆算シミュレーター</span>
            </a>
            <a href="4_第二の日置さん_思考ドリル.html" class="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-xs font-semibold flex items-center space-x-1.5">
            ...
            dataToRender.forEach((m, idx) => {{
                ...
                card.innerHTML = `
                    ...
                    <div class="space-y-1 pt-2 border-t border-slate-100">
                        ...
                        <ul class="text-xs text-slate-700 space-y-1 list-disc list-inside">${{decisionsHtml}}</ul>
                    </div>` : ''}}
...
'''
    ...
    # ==========================================
    # PAGE 4: 4_第二の日置さん_思考ドリル.html
    # ==========================================
    p4_content = f'''{get_head("4. 思考ドリル")}
    {get_header_html(3, total_count)}
    <main class="max-w-6xl mx-auto px-4 lg:px-8 py-8 space-y-8 bg-white">
        <div class="space-y-1">
            <h2 class="text-2xl font-bold text-slate-900 tracking-tight">
                4. 「第二の日置さん」思考ドリル
        ...
        <div class="flex justify-between items-center pt-6 border-t border-slate-100">
            <a href="3_全会議クロニクル.html" class="px-4 py-2 border border-slate-200 hover:bg-slate-50 text-slate-700 rounded-lg text-xs font-semibold flex items-center space-x-1.5">
                <i data-lucide="arrow-left" class="w-3.5 h-3.5"></i>
                <span>前へ: 3. 全会議クロニクル</span>
            </a>
            <a href="5_ドキュメント投入_同期.html" class="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-xs font-semibold flex items-center space-x-1.5">
...
'''
    ...
    # ==========================================
    # PAGE 5: 5_ドキュメント投入_同期.html
    # ==========================================
    p5_content = f'''{get_head("5. ドキュメント投入 / 同期")}
    {get_header_html(4, total_count)}
    <main class="max-w-6xl mx-auto px-4 lg:px-8 py-8 space-y-8 bg-white">
        <div class="space-y-1">
            <h2 class="text-2xl font-bold text-slate-900 tracking-tight">
                5. ドキュメント投入 ＆ 全体自動同期
            ...
            <p class="text-xs text-slate-500">
                PDF、Word（DOCX）、TXT形式の会議メモや議事録をドロップすると、全自動で解析され、全ページのデータが最新化されます。
            </p>
        </div>

        <div id="drop-zone" class="border-2 border-dashed border-slate-300 hover:border-slate-900 p-10 rounded-xl text-center space-y-4 transition cursor-pointer bg-white" onclick="document.getElementById('file-drop-input').click()">
            <input type="file" id="file-drop-input" accept=".pdf,.txt,.docx,.doc" multiple class="hidden" onchange="handleFileDropSelect(event)">
            <div class="w-14 h-14 rounded-xl bg-slate-100 text-slate-700 flex items-center justify-center mx-auto border border-slate-200">
            ...
            <div class="space-y-1">
                ...
                <p class="text-xs text-slate-500">
                    クリックしてファイルを選択（即座に全ページのデータが自動更新されます）
                </p>
            </div>
        ...
        <div class="border border-slate-200 rounded-xl p-6 space-y-4 bg-white">
        ...
        <div class="flex justify-between items-center pt-6 border-t border-slate-100">
            <a href="4_第二の日置さん_思考ドリル.html" class="px-4 py-2 border border-slate-200 hover:bg-slate-50 text-slate-700 rounded-lg text-xs font-semibold flex items-center space-x-1.5">
                <i data-lucide="arrow-left" class="w-3.5 h-3.5"></i>
    ...
    <script>
        ...
        async function handleFiles(files) {{
            ...
            if (addedCount > 0) {{
                ...
                alert(`${{addedCount}} 件のドキュメントを解析しました！「3. 全会議クロニクル」に移動します。`);
                window.location.href = "3_全会議クロニクル.html";
            }}
        }}
...
'''
    ...
    # Write all 5 HTML files to project root
    files_to_write = [
        (main_output_path, p1_content),
        (os.path.join(project_root, "index.html"), p1_content),
        (os.path.join(project_root, "1_人物像_CFO思考OS.html"), p1_content),
        (os.path.join(project_root, "2_必見KPI_逆算シミュレーター.html"), p2_content),
        (os.path.join(project_root, "3_全会議クロニクル.html"), p3_content),
        (os.path.join(project_root, "4_第二の日置さん_思考ドリル.html"), p4_content),
        (os.path.join(project_root, "5_ドキュメント投入_同期.html"), p5_content),
    ]

    for path, content in files_to_write:
        with open(path, 'w', encoding='utf-8') as fp:
            fp.write(content)
        print(f"  ✓ 生成完了: {os.path.basename(path)}")
