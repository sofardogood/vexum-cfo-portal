...
import { 
  ...
  SunMoon, Download, UploadCloud, Search, Calculator, Sparkles, 
  Cpu, Layers, CheckCircle, Terminal
} from 'lucide-react';
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100 transition-colors duration-150">
      ...
      {/* Top Clean Header */}
      <header className="sticky top-0 z-50 bg-white border-b border-slate-200 px-4 lg:px-8 py-3 dark:bg-slate-900 dark:border-slate-800 transition-colors shadow-sm">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="flex items-center space-x-2">
                ...
                <span className="px-2 py-0.5 text-xs font-semibold rounded bg-slate-100 text-slate-700 border border-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:border-slate-700">
          ...
          <div className="flex items-center space-x-2">
            <button 
              onClick={toggleTheme} 
              className="px-3 py-1.5 rounded-lg bg-white border border-slate-300 hover:bg-slate-100 text-slate-700 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-700 transition text-xs font-medium flex items-center space-x-1.5 shadow-sm"
              title="ライト / ダーク 切り替え"
            >
            ...
            <button 
              ...
              className="px-3 py-1.5 rounded-lg bg-slate-900 hover:bg-slate-800 text-white dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-slate-200 transition text-xs font-semibold flex items-center space-x-1.5 shadow-sm"
            >
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-6 lg:p-8 dark:bg-slate-900 dark:border-slate-800 shadow-sm space-y-6">
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                <div className="space-y-3 max-w-3xl">
                  <div className="inline-flex items-center space-x-2 px-2.5 py-1 rounded bg-slate-100 text-slate-800 text-xs font-semibold dark:bg-slate-800 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
                    <span>株式会社VEXUM 代表取締役 兼 経営統括・CFO</span>
                  </div>
                  <h2 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">
                    日置 佑輔 <span className="text-lg font-normal text-slate-500">（Yusuke）</span>
                  </h2>
                  <p className="text-base text-slate-900 dark:text-slate-100 font-bold leading-relaxed">
                ...
                <div className="grid grid-cols-2 gap-3 w-full md:w-auto shrink-0 font-mono text-center">
                  <div className="bg-slate-50 border border-slate-200 dark:bg-slate-950 dark:border-slate-800 p-3.5 rounded-xl shadow-sm">
                    <div className="text-[11px] text-slate-500 font-sans font-medium">全社目標 (1年後)</div>
                    <div className="text-lg font-bold text-slate-900 dark:text-white">有料 3,000社</div>
                    <div className="text-[10px] text-slate-500 font-sans">1億円ボーナス</div>
                  </div>
                  <div className="bg-slate-50 border border-slate-200 dark:bg-slate-950 dark:border-slate-800 p-3.5 rounded-xl shadow-sm">
                    ...
                    <div className="text-[10px] text-slate-500 font-sans">シリーズA</div>
                  </div>
                  <div className="bg-slate-50 border border-slate-200 dark:bg-slate-950 dark:border-slate-800 p-3.5 rounded-xl shadow-sm">
                    <div className="text-[11px] text-slate-500 font-sans font-medium">蓄積会議データ</div>
                    <div className="text-lg font-bold text-slate-900 dark:text-white">{dynamicStats.totalMeetings} 回分</div>
                    <div className="text-[10px] text-slate-500 font-sans">最新: {dynamicStats.latestDate}</div>
                  </div>
                  <div className="bg-slate-50 border border-slate-200 dark:bg-slate-950 dark:border-slate-800 p-3.5 rounded-xl shadow-sm">
                    ...
                    <div className="text-[10px] text-slate-500 font-sans">LINE即時承認</div>
                  </div>
                </div>
              </div>

              {/* Dynamic Latest Decision Banner */}
              <div className="bg-slate-50 border-l-4 border-slate-900 p-3.5 rounded-r-lg dark:bg-slate-950 dark:border-slate-100 flex items-start space-x-3 border border-slate-200 dark:border-slate-800">
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-6 lg:p-8 dark:bg-slate-900 dark:border-slate-800 shadow-sm space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="space-y-4 bg-slate-50 p-4 rounded-xl border border-slate-200 dark:bg-slate-950 dark:border-slate-800 shadow-sm">
                  <div>
                    ...
                    <div className="relative">
                      <input 
                        ...
                        className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-slate-900 font-mono font-bold text-lg focus:border-slate-900 focus:outline-none dark:bg-slate-900 dark:border-slate-700 dark:text-white shadow-sm" 
                      />
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-white border border-slate-300 rounded-lg px-3 py-1.5 text-slate-900 font-mono text-sm dark:bg-slate-900 dark:border-slate-700 dark:text-white focus:outline-none shadow-sm" 
                    />
                  </div>
                </div>

                <div className="md:col-span-2 grid grid-cols-2 sm:grid-cols-3 gap-3">
                  <div className="bg-slate-50 border border-slate-200 dark:bg-slate-950 dark:border-slate-800 p-4 rounded-xl flex flex-col justify-between shadow-sm">
                    <div className="text-xs font-bold text-slate-700 dark:text-slate-300">① 必要初期設計数 (先行指標)</div>
                    <div className="text-3xl font-extrabold text-slate-900 dark:text-white font-mono my-2">{requiredDesign}</div>
                    <div className="text-[10px] text-slate-500 font-medium">リードタイム: 約22日前</div>
                  </div>

                  <div className="bg-slate-50 border border-slate-200 dark:bg-slate-950 dark:border-slate-800 p-4 rounded-xl flex flex-col justify-between shadow-sm">
                    <div className="text-xs font-bold text-slate-700 dark:text-slate-300">② 必要アポイント数</div>
                    <div className="text-3xl font-extrabold text-slate-900 dark:text-white font-mono my-2">{requiredAppos}</div>
                    <div className="text-[10px] text-slate-500 font-mono font-medium">日次: 約{dailyAppos}件</div>
                  </div>

                  <div className="bg-slate-50 border border-slate-200 dark:bg-slate-950 dark:border-slate-800 p-4 rounded-xl flex flex-col justify-between shadow-sm">
                    <div className="text-xs font-bold text-slate-700 dark:text-slate-300">③ 必要見込み (3倍ルール)</div>
                    <div className="text-3xl font-extrabold text-slate-900 dark:text-white font-mono my-2">{pipeline3x}</div>
                    <div className="text-[10px] text-slate-600 font-bold">下振れ防止バッファ</div>
                  </div>

                  <div className="bg-slate-50 border border-slate-200 dark:bg-slate-950 dark:border-slate-800 p-4 rounded-xl flex flex-col justify-between shadow-sm">
                    <div className="text-xs font-bold text-slate-700 dark:text-slate-300">④ 必要リーダー数</div>
                    <div className="text-3xl font-extrabold text-slate-900 dark:text-white font-mono my-2">{requiredLeaders}</div>
                    <div className="text-[10px] text-slate-500 font-medium">名 (案件管理体制)</div>
                  </div>

                  <div className="bg-slate-50 border border-slate-200 dark:bg-slate-950 dark:border-slate-800 p-4 rounded-xl flex flex-col justify-between shadow-sm">
                    <div className="text-xs font-bold text-slate-700 dark:text-slate-300">⑤ 想定月次売上 (単価25万)</div>
                    <div className="text-3xl font-extrabold text-slate-900 dark:text-white font-mono my-2">{monthlySalesMan}万</div>
                    <div className="text-[10px] text-slate-500 font-medium">円 / 月</div>
                  </div>

                  <div className="bg-slate-50 border border-slate-200 dark:bg-slate-950 dark:border-slate-800 p-4 rounded-xl flex flex-col justify-between shadow-sm">
                    <div className="text-xs font-bold text-slate-700 dark:text-slate-300">⑥ インターン給与上限 (50%)</div>
                    <div className="text-3xl font-extrabold text-slate-900 dark:text-white font-mono my-2">{internBudgetMan}万</div>
                    <div className="text-[10px] text-slate-500 font-medium">円 (財務規律上限)</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-white border border-slate-200 p-5 rounded-xl dark:bg-slate-900 dark:border-slate-800 shadow-sm space-y-3">
                <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2">
                  ...
                  <span className="text-[10px] text-slate-500 font-mono">Sales Funnel</span>
                </div>
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="bg-slate-50 p-2.5 rounded dark:bg-slate-950 border border-slate-200 dark:border-slate-800">
                    <div className="text-slate-600 dark:text-slate-400">初回接触・リード数</div>
                    <div className="font-bold font-mono text-slate-900 dark:text-white text-sm">月200件</div>
                  </div>
                  <div className="bg-slate-50 p-2.5 rounded dark:bg-slate-950 border border-slate-200 dark:border-slate-800">
                    <div className="text-slate-600 dark:text-slate-400">初期設計数【先行指標】</div>
                    <div className="font-bold font-mono text-slate-900 dark:text-white text-sm">転換率 約80%</div>
                  </div>
                  <div className="bg-slate-50 p-2.5 rounded dark:bg-slate-950 border border-slate-200 dark:border-slate-800">
                    <div className="text-slate-600 dark:text-slate-400">上駐開始数【必達マスト】</div>
                    <div className="font-bold font-mono text-slate-900 dark:text-white text-sm">月64〜100件</div>
                  </div>
                  <div className="bg-slate-50 p-2.5 rounded dark:bg-slate-950 border border-slate-200 dark:border-slate-800">
                    <div className="text-slate-600 dark:text-slate-400">見込みパイプライン</div>
                    <div className="font-bold font-mono text-slate-900 dark:text-white text-sm">目標の3倍 (150%以上)</div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white border border-slate-200 p-5 rounded-xl dark:bg-slate-900 dark:border-slate-800 shadow-sm space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="bg-slate-50 p-2.5 rounded dark:bg-slate-950 border border-slate-200 dark:border-slate-800">
                    <div className="text-slate-600 dark:text-slate-400">解約率 (チャーン)</div>
                    <div className="font-bold font-mono text-slate-900 dark:text-white text-sm">約70% (要改善)</div>
                  </div>
                  <div className="bg-slate-50 p-2.5 rounded dark:bg-slate-950 border border-slate-200 dark:border-slate-800">
                    <div className="text-slate-600 dark:text-slate-400">全社原価率</div>
                    <div className="font-bold font-mono text-slate-900 dark:text-white text-sm">約66% (圧縮目標)</div>
                  </div>
                  <div className="bg-slate-50 p-2.5 rounded dark:bg-slate-950 border border-slate-200 dark:border-slate-800">
                    <div className="text-slate-600 dark:text-slate-400">インターン給与規律</div>
                    <div className="font-bold font-mono text-slate-900 dark:text-white text-sm">売上の50%以内</div>
                  </div>
                  <div className="bg-slate-50 p-2.5 rounded dark:bg-slate-950 border border-slate-200 dark:border-slate-800">
                    <div className="text-slate-600 dark:text-slate-400">CRM・財務乖離</div>
                    <div className="font-bold font-mono text-slate-900 dark:text-white text-sm">乖離ゼロ (完全一致)</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                <input 
                  ...
                  className="bg-white border border-slate-300 rounded-lg px-3 py-1.5 text-xs text-slate-900 placeholder-slate-400 focus:border-slate-900 focus:outline-none dark:bg-slate-900 dark:border-slate-700 dark:text-white shadow-sm" 
                />
                <select 
                  value={selectedTag} 
                  onChange={e => setSelectedTag(e.target.value)} 
                  className="bg-white border border-slate-300 rounded-lg px-2.5 py-1.5 text-xs text-slate-800 dark:bg-slate-900 dark:border-slate-700 dark:text-slate-200 focus:outline-none shadow-sm"
                >
            ...
            <div className="space-y-4">
              {filteredChronicle.map((m, idx) => (
                <div key={idx} className="bg-white border border-slate-200 rounded-xl p-5 dark:bg-slate-900 dark:border-slate-800 shadow-sm space-y-3">
                  <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2">
                    <div className="flex items-center space-x-2">
                      <span className="px-2 py-0.5 rounded bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-slate-200 text-xs font-bold font-mono border border-slate-200 dark:border-slate-700">
                        第{idx + 1}回
                      </span>
                    ...
                    <div className="flex gap-1">
                      {(m.tags || []).map((t: string, i: number) => (
                        <span key={i} className="px-2 py-0.5 rounded text-[10px] bg-slate-50 border border-slate-200 text-slate-700 dark:bg-slate-950 dark:border-slate-800 dark:text-slate-300 font-medium">
                          {t}
                        </span>
                      ))}
                    </div>
                  ...
                  <p className="text-xs text-slate-700 dark:text-slate-300 leading-relaxed font-normal">{m.summary}</p>
                  
                  {m.yusuke_decisions && m.yusuke_decisions.length > 0 && (
                    <div className="space-y-1 pt-2 border-t border-slate-100 dark:border-slate-800">
                      <div className="text-[11px] font-bold text-slate-900 dark:text-white">日置さんの重要意思決定・指摘：</div>
                      <ul className="text-xs text-slate-700 dark:text-slate-300 space-y-1 list-disc list-inside font-normal">
                  ...
                  {m.quotes && m.quotes.length > 0 && (
                    <div className="space-y-1 pt-1">
                      {m.quotes.map((q: string, qi: number) => (
                        <div key={qi} className="bg-slate-50 border-l-2 border-slate-900 p-2.5 rounded text-xs text-slate-800 dark:bg-slate-950 dark:border-slate-100 dark:text-slate-200 italic font-medium">
                          💬 "{q}"
                        </div>
                      ))}
                    </div>
        ...
        {activeTab === 'roadmap' && (
          <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs max-h-60 overflow-y-auto pr-1">
                {dynamicStats.allDecisions.slice(0, 10).map((dec, i) => (
                  <div key={i} className="bg-slate-50 p-2.5 rounded border border-slate-200 dark:bg-slate-950 dark:border-slate-800 space-y-1">
                    <div className="text-[10px] font-bold text-slate-600 dark:text-slate-400">{dec.meetingDate}</div>
                    <div className="text-slate-800 dark:text-slate-200 font-normal">{dec.text}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-6 dark:bg-slate-900 dark:border-slate-800 shadow-sm space-y-6">
              ...
              <div className="space-y-6">
                {baseDrills.map(d => (
                  <div key={d.id} className="bg-slate-50 p-5 rounded-xl border border-slate-200 dark:bg-slate-950 dark:border-slate-800 space-y-3 shadow-sm">
                    ...
                    <p className="text-sm font-bold text-slate-900 dark:text-white">{d.question}</p>
                        ...
                        <button 
                          ...
                          className={`w-full text-left p-3 rounded-lg border transition ${
                            drillAnswers[d.id] === opt.key 
                              ? (opt.key === d.correct ? 'bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900 font-bold' : 'bg-rose-100 text-rose-900 border-rose-300 dark:bg-rose-950 dark:text-rose-200 dark:border-rose-800 font-medium') 
                              : 'bg-white border-slate-300 hover:bg-slate-100 text-slate-800 dark:bg-slate-900 dark:border-slate-700 dark:text-slate-200 font-medium shadow-sm'
                          }`}
                        >
                    ...
                    {drillAnswers[d.id] && (
                      <div className={`p-3 rounded-lg text-xs leading-relaxed ${
                        drillResults[d.id] ? 'bg-slate-100 text-slate-800 dark:bg-slate-900 dark:text-slate-200 border border-slate-300 dark:border-slate-700 font-medium' : 'bg-rose-50 text-rose-800 dark:bg-rose-950/40 dark:text-rose-200 border border-rose-200 dark:border-rose-900 font-medium'
                      }`}>
                        {d.explanation}
                      </div>
                    )}
        ...
        {activeTab === 'manage' && (
          <div className="space-y-8">
            ...
            >
              ...
              />
              <div className="w-14 h-14 rounded-xl bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300 flex items-center justify-center mx-auto border border-slate-200 dark:border-slate-700">
      ...
      {/* Clean Footer */}
      <footer className="border-t border-slate-200 dark:border-slate-800 mt-16 py-8 px-4 text-center text-xs text-slate-600 dark:text-slate-400">
